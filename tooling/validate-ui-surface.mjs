...
export async function runUiValidation({ baseUrl, outputDir }) {
  ...
  try {
    ...
    await page.getByRole('heading', { name: 'Dette test' }).waitFor()
    await page.getByLabel('Montant (€)').first().fill('100')
    await page.getByLabel('Date precise').first().fill('2026-03-15')
    await page.getByRole('button', { name: /valider le paiement/i }).click()
    await page.getByRole('link', { name: /import & sauvegarde/i }).click()
    await page.getByRole('heading', { name: /centre de confiance/i }).waitFor()
